...
#define NUM_MOTORS 2 // Update the number of motors
...
void followTrajectory() {
    ...
    if (freshWirelessData) {
        //changed the max_forward and max turn negative signs
        double forward = abs(controllerMessage.joystick1.y) < 0.15 ? 0 : mapDouble(controllerMessage.joystick1.y, -1, 1, MAX_FORWARD, -MAX_FORWARD);
        double turn = abs(controllerMessage.joystick1.x) < 0.15 ? 0 : mapDouble(controllerMessage.joystick1.x, -1, 1, -MAX_TURN, MAX_TURN);
